...
class UrbanEcho(mlflow.pyfunc.PythonModel):
    ...
    def preprocess(self, input):
        ...
        scaled_feature = librosa.feature.mfcc(
            y=samples, sr=audio.frame_rate, n_mfcc=self.n_mfcc
        )
    ...
    def postprocess(self, input):
        ...
        # print(prediction)
        return {
            "class": self.classes[prediction.argmax().item()],
...
def create_model_signature(model, input_shape):
    ...
    signature = mlflow.models.infer_signature(
        input_sample.detach().numpy(), model(input_sample).detach().numpy()
    )
    return signature
...
def parse_args():
    ...
    parser.add_argument(
        "--artifact_path", type=str, required=True, help="Artifact path of the model"
    )
    parser.add_argument(
        "--input_shape",
        type=int,
    ...
    parser.add_argument(
        "--classes", type=str, default="classes.txt", help="Path to the classes file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="URI of the tracking server",
    )
    parser.add_argument(
        "--experiment_name", type=str, default="Default", help="Name of the experiment"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Model {args.model_name} registered successfully! Run ID: {args.run_id} Experiment: {args.experiment_name} Tracking URI: {args.tracking_uri}"
    )
